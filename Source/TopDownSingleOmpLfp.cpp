...
const char TopDownSingleOmpLfp::SOURCE = -1;
const char TopDownSingleOmpLfp::LINK = -2;


CellLocation TopDownSingleOmpLfp::sourceForLocation(int row, int col, const FramedMatrix<PathData>& pathMatrix, const InletNumberMatrix& inletNumberMatrix)
{
  return (inletNumberMatrix.value[row][col] != SOURCE) ? pathMatrix.value[row][col].source : CellLocation{row, col};
}


void TopDownSingleOmpLfp::compareAndUpdate(const CellLocation& source, const FramedMatrix<PathData>& pathMatrix, FlowPathLength& maxLength, CellLocation& maxSource)
{
...
TopDownSingleOmpLfp::IntermediateResult TopDownSingleOmpLfp::mainAlgorithm(const FlowDirectionMatrix& directionMatrix)
{
  const int height = directionMatrix.height;
  const int width = directionMatrix.width;

  InletNumberMatrix inletNumberMatrix = InletNumberCalculator::calculateParallel(directionMatrix, SOURCE, LINK);
...
CellLocation TopDownSingleOmpLfp::execute(const FlowDirectionMatrix& directionMatrix, const CellLocation& outlet)
{
...
std::vector<CellLocation> TopDownSingleOmpLfp::executeMultiple(const FlowDirectionMatrix& directionMatrix, const std::vector<CellLocation>& outlet)
{
  IntermediateResult result = mainAlgorithm(directionMatrix);
